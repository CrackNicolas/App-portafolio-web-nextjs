...
export default function ComponentItem(props: Props) {
    ...
    return (
        <div className="relative grid xl:grid-cols-5 gap-x-[40px] gap-y-[10px] sm:gap-y-[15px]">
            <span className={`${animate ? 'animate-[presentationLeft_1.2s_ease-in-out]' : 'opacity-0'} text-primary transition duration-500 xl:col-span-1 text-end tracking-wider my-auto font-semibold text-[15px]`}>
                {t(`experience.data.item_${number}.title`)}
            </span>
            <div className={`${animate ? 'animate-[presentationBottom_1s_ease-in-out]' : 'opacity-0'} text-primary transition duration-500 xl:col-span-4 xl:border-l-[3px] border-secondary xl:pt-[20px] xl:pr-[5px] xl:pb-[10px] xl:pl-[50px]`}>
                ...
                <div className={`hover:bg-gradient-to-r from-primary to-secondary ${animate ? 'animate-[presentationRight_1.2s_ease-in-out]' : 'opacity-0'} xl:mt-[-10px] group cursor-pointer hover:bg-secondary hover:text-tertiary hover:shadow-none transition duration-700 shadow-3xl shadow-primary pt-[5px] pb-[10px] px-[10px] sm:px-[15px] rounded-md leading-7 sm:leading-8 border-[1px] border-secondary`}>
